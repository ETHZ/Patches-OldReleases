...
float EcalClusterLocalContCorrection::getValue( const reco::BasicCluster & basicCluster) const
{
  checkInit();
  ...
  //If not barrel, return 1:
  if (TMath::Abs(basicCluster.eta()) >1.4442 ) return 1.;
  ...
  //search which crystal is closest to the cluster position and call it crystalseed:
  //std::vector<DetId> crystals_vector = basicCluster.getHitsByDetId();   //deprecated
